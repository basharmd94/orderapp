...
export default function NavCollapse({ menu, level, parentId }) {
  ...
  const menus = menu.children?.map((item) => {
    const key = item.key || item.id || `${menu.id}-${item.title}`;
    
    switch (item.type) {
      case 'collapse':
        return <NavCollapse key={key} menu={item} level={level + 1} parentId={parentId} />;
      case 'item':
        return <NavItem key={key} item={item} level={level + 1} />;
      default:
        return (
